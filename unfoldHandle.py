...
class unfoldHandle(object):
	...
	def doUnfold(self,data,back=0,regdiff=0,iteration=0):
		...
		if regdiff!=0:
			Unfold=RooUnfoldBayes(self.response,data,1,False,self.name+"_reg",self.name+"_reg")
			h2=data.Clone()
        		for i in range(100):
					...
					h2=h1.Clone()
                        		Unfold.Reset()
                        		Unfold=RooUnfoldBayes(self.response,data,i+2,False,self.name,self.name)
		elif iteration !=0:
			s="iter_%d"%iteration
			Unfold=RooUnfoldBayes(self.response,data,iteration,False,self.name+s,self.name+s)
			h1=Unfold.Hreco().Clone()
			return h1
		else:
			Unfold=RooUnfoldInvert(self.response,data,self.name+"_invert",self.name+"_invert")
			h1=Unfold.Hreco().Clone()
			self.UnfoldedData=h1
			return h1
